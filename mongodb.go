...
type MongoDBConfig func(*options.ClientOptions)
...
func UseUUID(opts *options.ClientOptions) {
	opts.SetRegistry(UUIDRegistry)
}

func newMongoClient(opts *options.ClientOptions) (*mongo.Client, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	return mongo.Connect(ctx, opts.SetRegistry(UUIDRegistry))
